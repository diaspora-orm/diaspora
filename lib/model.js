...
class Model {
	constructor( namespace, name, modelDesc ) {
		...
		const sourcesNormalized = c.array(modelDesc.sources) ? _.zipObject(modelDesc.sources, _.times(modelDesc.sources.length, _.constant({}))) : _.mapValues(modelDesc.sources, (remap, dataSourceName) => {
			if(true === remap){
				return {};
		...
		_.forEach(sourcesNormalized, (remap, sourceName) => {
			...
			console.log({sourceConfiguring, name, remap});
