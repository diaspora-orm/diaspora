...
function findArgs( queryFind = {}, options = {}, dataSourceName ) {
	let ret;
	...
	ret.dataSource = this.getDataSource( ret.dataSourceName );
	return ret;
}
...
class Model {
	...
	find( queryFind, options, dataSourceName ) {
		const args = findArgs.call( this, queryFind, options, dataSourceName );
		return args.dataSource.findOne( this.name, args.queryFind, args.options ).then( dataSourceEntity => {
	...
	findMany( queryFind, options, dataSourceName ) {
		const args = findArgs.call( this, queryFind, options, dataSourceName );
		return args.dataSource.findMany( this.name, args.queryFind, args.options ).then( entities => {
			const newEntities = _.map( entities, entity => new this.entityFactory( entity ));
			const collection = new Set( this, newEntities );
			return Promise.resolve( collection );
		});
	...
	update( queryFind, update, options = {}, dataSourceName ) {
		const args = findArgs.call( this, queryFind, options, dataSourceName );
		return args.dataSource.updateOne( this.name, args.queryFind, update, args.options ).then( dataSourceEntity => {
			if ( _.isNil( dataSourceEntity )) {
				return Promise.resolve();
			}
			const newEntity = new this.entityFactory( dataSourceEntity );
	...
	updateMany( queryFind, update, options = {}, dataSourceName ) {
		const args = findArgs.call( this, queryFind, options, dataSourceName );
		return args.dataSource.updateMany( this.name, args.queryFind, update, args.options ).then( entities => {
			const newEntities = _.map( entities, entity => new this.entityFactory( entity ));
			const collection = new Set( this, newEntities );
			return Promise.resolve( collection );
		});
	...
	delete( queryFind, options = {}, dataSourceName ) {
		const args = findArgs.call( this, queryFind, options, dataSourceName );
		return args.dataSource.deleteOne( this.name, args.queryFind, args.options );
	}
	...
	deleteMany( queryFind = {}, options = {}, dataSourceName ) {
		const args = findArgs.call( this, queryFind, options, dataSourceName );
		return args.dataSource.deleteMany( this.name, args.queryFind, args.options );
	}
}
