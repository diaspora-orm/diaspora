...
const validateArrayItems = ( validator, fieldDesc, keys ) => {
	console.log('Validate array items', keys)
...
class PathStack {
	 ...
	 */
	pushEntityProp( ...prop ) {
		this.segmentsEntity = _(this.segmentsEntity).concat( prop ).compact().value();
		return this;
	}
	...
	pushValidationProp( ...prop ) {
		this.segmentsValidation = _(this.segmentsValidation).concat( prop ).compact().value();
		return this;
	}
...
class Validator {
	...
	check( value, keys, options = {}) {
		...
		console.log('Check', {value, keys, options});
		...
		_.forEach( VALIDATION_STEPS, (validationStep, index) => {
			console.log({validationStep, index})
