...
function wrapEventsAction( sourceName, action, verb ) {
	const getVerb = index => _.isArray( verb ) ? verb[index] : verb;
	return Promise.all( this.entities.map(( entity, index ) => entity.emit( `before${ getVerb( index ) }` ))).then(() => {
		return Promise.all( this.entities.map( entity => entity[action]( sourceName, {
			skipEvents: true,
		})));
	}).then(() => {
		return Promise.all( this.entities.map(( entity, index ) => entity.emit( `after${ getVerb( index ) }` )));
	});
}
...
class Set {
	...
	persist( sourceName ) {
		...
		return Promise.all( this.entities.map( entity => entity.emit( 'beforePersist' ))).then(() => {
			...
			return wrapEventsAction.call( this, sourceName, 'persist', _.map( suffixes, suffix => `Persist${ suffix }` ));
		}).then(() => {
