...
const maybeEmit = (entity, event, options) => {
	if ( options.skipEvents ) {
		return Promise.resolve(entity);
	} else {
		entity.emit( event, sourceName ).then(() => entity);
	}
}
/**
		...
		constructor( source = {}) {
			...
			const entityPrototype = {
				...
				fetch( sourceName, options = {}) {
					...
					const beforeState = state;
					state = 'syncing';
					const promise = maybeEmit(this, 'beforeFetch', options);
					return promise.then(() => {
						// Depending on state, we are going to perform a different operation
						if ( 'orphan' === beforeState ) {
						...
						this.dataSources[dataSource.name] = dataStoreEntity;
						attributes = dataStoreEntity.toObject();
						return maybeEmit(this, 'afterFetch', options);
					});
				},
				/**
				...
				destroy( sourceName, options = {}) {
					...
					const beforeState = state;
					state = 'syncing';
					const promise = maybeEmit(this, 'beforeDestroy', options);
					return promise.then(() => {
						if ( 'orphan' === beforeState ) {
							return Promise.reject( new EntityStateError( 'Can\'t fetch an orphan entity.' ));
						} else {
						...
						return maybeEmit(this, 'afterDestroy', options);
					});
				},
				/**
