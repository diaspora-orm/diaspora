...
const ensureAllEntities = () => {
	...
	return results => {
		const mappedResults = results = _(results).castArray().map( filterResults );
		if ( _.isArrayLike( results )) {
			return Promise.resolve(mappedResults.value());
		} else {
			return Promise.resolve(mappedResults.get(0));
		}
	}
};
