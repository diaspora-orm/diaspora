...
const VALIDATE_ARRAY_ITEMS = ( fieldDesc, keys ) => {
	return ( propVal, propName ) => {
		if ( fieldDesc.hasOwnProperty( 'of' )) {
			const ofArray = _.castArray( fieldDesc.of );
			const subErrors = _( ofArray ).map( desc => Diaspora.checkField( propVal, desc, _.concat( keys, [ propName ])));
			if ( !_.isArray( fieldDesc.of )) {
				return subErrors.get( 0 );
			} else if ( subErrors.compact().length > 0 ) {
	...
	TYPE: {
		...
		array( keys, fieldDesc, value ) {
			if ( !_.isArray( value )) {
				...
				const deepTest = _.isObject(
					fieldDesc.of
				) ? _( value ).map( VALIDATE_ARRAY_ITEMS( fieldDesc, keys ))
						.omitBy( _.isEmpty )
						.value() : {};
				if ( !_.isEmpty( deepTest )) {
					return {children: deepTest};
...
const VALIDATION_STEPS = {
	...
	CHECK_TYPE_REQ( error, fieldDesc ) {
...
const Diaspora = {
	...
	checkField( value, fieldDesc, keys ) {
		...
		VALIDATION_STEPS.CHECK_CUSTOM( error, fieldDesc );

		// Check the type and the required status
		VALIDATION_STEPS.CHECK_TYPE_REQ( error, fieldDesc );

		// Check enum values
		VALIDATION_STEPS.CHECK_ENUM( error, fieldDesc );
