	...
	if ( !process.browser ) {
		...
		if ( process.env.NODE_ENV !== 'production' ) {
			...
			const formatDate = (date = new Date()) => {
				return `${td(date.getFullYear())}/${td(date.getMonth() + 1)}/${td(date.getDay())} ${td(date.getHours())}:${td(date.getMinutes())}:${td(date.getSeconds())}`;
			};
			...
			log.add( new transports.Console({
				format: combine(
					...
					format((infos, opts) => {
						...
						let message = `${infos.level.replace(level, 'Diaspora: ' + level)}${log.paddings[level]}@${formatDate()} => ${infos.message}`;
		...
		// log.http('Logger started');
		// log.verbose('Logger started');
		// log.debug('Logger started');
		// log.silly('Logger started');
		return log;
	} else {
		return console;
	}
...
const Diaspora = {
	...
	*/
	createDataSource( adapterLabel, config, ...otherArgs ) {
		if ( !adapters.hasOwnProperty( adapterLabel )) {
			...
			try {
				try{
					...
					throw new Error( `Unknown adapter "${ adapterLabel }" (expected in module "${moduleName}"). Available currently are ${ Object.keys( adapters ).join( ', ' ) }. Additionnaly, an error was thrown: ${ e }` );
				}
				require( moduleName );
			} catch ( e ) {
		...
		const baseAdapter = new adapters[adapterLabel]( config, ...otherArgs );
	...
	* @returns {Adapters.DiasporaAdapter} New adapter spawned.
	*/
	createNamedDataSource( sourceName, adapterLabel, configHash, ...otherArgs ) {
		const dataSource = Diaspora.createDataSource( adapterLabel, configHash, ...otherArgs );
