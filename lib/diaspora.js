			...
			get: function( target, key ) {
				...
				if ( _.isString(key)){
					...
					if( method = key.match( /^(find|update|insert|delete)(Many|One)$/ )) {
						console.log({method, adapterRemap})
						method[2] = method[2].toLowerCase();
						method = _.mapKeys( method.slice( 0, 3 ), ( val, key ) => {
							return [ 'full', 'query', 'number' ][key];
						});
						return wrapDataSourceAction( target[key], method, target, adapterRemap );
					} else if(key === 'configureCollection'){
						console.log('Catched configureCollection');
						return (tableName, remapping) => {
							_.assign(adapterRemap, remapping);
							_.assign(adapterRemapInv, _.invert(remapping));
							return target.configureCollection(tableName, remapping);
						};
					}
				}
				return target[key];
