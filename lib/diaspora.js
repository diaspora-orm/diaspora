...
const VALIDATIONS = {
	TYPE: {
					...
					string(fieldDesc, value){
						if ( !_.isString( value )) {
							return {type: `${ keys.join( '.' ) } expected to be a "${ fieldDesc.type }"`};
						}
					},

					integer(fieldDesc, value){
						if ( !_.isInteger( value )) {
							return {type: `${ keys.join( '.' ) } expected to be a "${ fieldDesc.type }"`};
						}
					},

					float(fieldDesc, value){
						if ( !_.isNumber( value )) {
							return {type: `${ keys.join( '.' ) } expected to be a "${ fieldDesc.type }"`};
						}
					},

					date(fieldDesc, value){
						if ( !_.isDate( value )) {
							return {type: `${ keys.join( '.' ) } expected to be a "${ fieldDesc.type }"`};
						}
					},

					object(fieldDesc, value){
						if ( !_.isObject( value )) {
							return {type: `${ keys.join( '.' ) } expected to be a "${ fieldDesc.type }"`};
						} else {
	...
	checkField( value, fieldDesc, keys ) {
		...
		if ( !_.isNil( fieldDesc.type ) && !_.isNil( fieldDesc.model )) {
			...
			if ( _.isString( fieldDesc.type )) {
				...
				_.assign(error, tester(fieldDesc, value));
			} else {
