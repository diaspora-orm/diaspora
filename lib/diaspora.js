...
class Diaspora {
	 ...
	 */
	static createDataSource( adapterLabel, config ) {
	...
	get Model(){
		return components.Model.bind(components.Model, this);
	}
	 ...
	 */
	static get adapters(){
		return Diaspora[PRIVATE].adapters;
	}
	 ...
	 */
	get dependencies(){
		return this[PRIVATE].dependencies;
	}
...
if(_.isFunction(Object.freeze)){
	_.forEach(['', 'Errors', 'Adapters'], path => {
		Object.freeze(_.get(components, path));
	});
}
