...
const validateArrayItems = (validator, fieldDesc, keys) => {
    return (propVal, index) => {
        if (fieldDesc.hasOwnProperty('of')) {
            ...
            const subErrors = _(ofArray).map((desc, subIndex) => validator.check(propVal, keys.clone().pushValidationProp('of', _.isArray(fieldDesc.of) ? subIndex : undefined).pushEntityProp(index), { getProps: false }));
    ...
    TYPE: {
        ...
        object(keys, fieldDesc, value) {
            if (!_.isObject(value)) {
                ...
                const deepTest = _.isObject(fieldDesc.attributes) ? _(_.assign({}, fieldDesc.attributes, value)).mapValues((pv, propName) => {
                    const propVal = value[propName];
                    return this.check(propVal, keys.clone().pushValidationProp('attributes').pushProp(propName), { getProps: false });
                })
                    .omitBy(_.isEmpty)
                    .value() : {};
        ...
        array(keys, fieldDesc, value) {
            if (!_.isArray(value)) {
                ...
                const deepTest = _.isObject(fieldDesc.of) ? _(value).map(validateArrayItems(this, fieldDesc, keys))
                    .omitBy(_.isEmpty)
                    .value() : {};
                if (!_.isEmpty(deepTest)) {
                    return { children: deepTest };
