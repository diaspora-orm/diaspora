...
class InMemoryDiasporaAdapter extends DiasporaAdapter {
	...
	deleteOne( table, queryFind, options = {} ) {
		...
		return this.findOne(table, queryFind, options).then(entityToDelete => {
			storeTable.items = l.reject( storeTable.items, entity => entity.id === entityToDelete.idHash[this.name] );
	...
	deleteMany( table, queryFind, options = {} ) {
		...
		return this.findMany(table, queryFind, options).then(entitiesToDelete => {
			const entitiesIds = l.map(entitiesToDelete, entity => l.get(entity, `idHash.${this.name}`));
			storeTable.items = l.reject( storeTable.items, entity => {
				return l.includes(entitiesIds, entity.id);
			} );
			return Promise.resolve( undefined );
		});
	}
