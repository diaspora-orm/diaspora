...
const httpRequest = async( method, endPoint, data, queryObject, stringifyQueryString = true ) => {
	if ( !process.browser ) {
		if ( _.isNil( data )) {
			data = true;
		}
		return await require( 'request-promise' )[method.toLowerCase()]( endPoint, {
			json: data,
			qs:   stringifyQueryString ? _.mapValues( queryObject, JSON.stringify ) : queryObject,
		});
	} else {
...
class WebApiDiasporaAdapter extends DiasporaAdapter {
	...
	*/
	constructor( config = {}, eventProviders = []) {
		...
		*/
		...
		_.map(eventProviders, eventProvider => eventProvider.forIn((val, key) => this.on(key, val)));
	...
	async httpRequest(...args){
		Diaspora.logger.silly('Querying api:', args);
		const response = await httpRequest(...args);
		Diaspora.logger.silly('Query results:', response);
		return response;
	};
	...
	*/
	httpQuery( verb, endPoint, data, queryObject ) {
		if(this.transformQueryString){
			queryObject = this.transformQueryString(queryObject);
		}
		return this.httpRequest(
			...
			data,
			queryObject,
			!_.isFunction(this.transformQueryString)
		);
	}
