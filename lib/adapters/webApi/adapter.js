...
const httpRequest = async (method, baseEndPoint, endPoint, data, queryObject) => {
	if(!process.browser){
		...
		return await request[method.toLowerCase()](baseEndPoint + endPoint, {json: data, qs: queryObject});
	} else {
...
const getQueryObject = (queryFind, options) => {
	...
	const queryObject = _.assign({}, _.omit(options, ['remapInput', 'remapOutput']), {where: queryFind});
	return _.mapValues(queryObject, JSON.stringify);
}
