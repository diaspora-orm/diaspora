...
class MongoDiasporaAdapter extends DiasporaAdapter {
	...
	findOne( table, queryFind, options = {}) {
		...
		const collection = this.db.collection( table );
		return collection.findOne( queryFind, options ).then( foundItem => {
	...
	updateOne( table, queryFind, update, options = {}) {
		...
		return this.findOne( table, queryFind, subOptions ).then( foundItem => {
			return collection.updateOne({
				_id: foundItem._id,
			}, _.assign({}, foundItem, update ), subOptions ).then(() => Promise.resolve( foundItem._id ));
	...
	updateMany( table, queryFind, update, options = {}) {
		...
		return this.findMany( table, queryFind, subOptions ).then( foundItems => {
			return Promise.map( foundItems, foundItem => {
				return collection.updateOne({
					_id: foundItem._id,
				}, _.assign({}, foundItem, update ), subOptions ).then(() => Promise.resolve( foundItem._id ));
			});
	...
	deleteOne( table, queryFind, options = {}) {
		...
		const collection = this.db.collection( table );
		return collection.deleteOne( queryFind, options ).then(() => Promise.resolve());
	}
