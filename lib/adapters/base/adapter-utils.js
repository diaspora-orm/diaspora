...
const validateOption = ( key, val, config ) => {
	...
	if ( config.rng ) {
		const range = config.rng.match( /^([[\]])((-)?(\d+|∞)),((-)?(\d+|∞))([[\]])$/ );
		if ( range ) {
			const lower = getNum( ...( range.splice( 2, 3 )));
			const upper = getNum( ...( range.splice( 2, 3 )));
			const isInRangeLower = '[' === range[1] ? val >= lower : val > lower;
			const isInRangeUpper = ']' === range[2] ? val <= upper : val < upper;
