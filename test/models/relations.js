...
let testModel1;
let testModel2;
const MODEL_NAME_1 = 'testModel1';
const MODEL_NAME_2 = 'testModel2';
const SOURCE = 'inMemory-relations';


it( 'Should create a model', () => {
	Diaspora.createNamedDataSource( SOURCE, 'inMemory' );
	testModel1 = Diaspora.declareModel( MODEL_NAME_1, {
		sources:    [ SOURCE ],
		schema:     false,
		attributes: {
			rel1to1: {
	...
	testModel2 = Diaspora.declareModel( MODEL_NAME_2, {
		sources:    [ SOURCE ],
		schema:     false,
		attributes: {
			foo: {
	...
	expect( testModel2 ).to.be.an( 'object' );
	if ( 'undefined' === typeof window ) {
		expect( testModel1.constructor.name ).to.be.eql( 'Model' );
		expect( testModel2.constructor.name ).to.be.eql( 'Model' );
	}
});
