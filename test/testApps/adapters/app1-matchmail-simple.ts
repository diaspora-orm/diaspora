export default (adapter, data, tableName) => {
	it('❌ Clear old data', () => {
		return adapter
			...
			.then(found => {
				expect(found)
					.to.be.a.set.of.dataStoreEntity(adapter)
	...
	it('✨ Insert test data', function insertTestData() {
		...
		return adapter.insertMany(tableName, data).then(entities => {
			expect(entities)
				.to.be.a.set.of.dataStoreEntity(adapter, data)
	...
	it(`🔃 Update those items with email (options ${chalk.bold(
		'skip'
	)} & ${chalk.bold('limit')})`, () => {
		return Promise.map(l.keys(ipRecords), ip => {
			const records = ipRecords[ip];
			const emails = l(records)
				.map('email')
				.value();
			let lastSyncIdx = 0;
			let email = l(emails)
	...
	const pageSize = 11;
	let page = 0;
	it(`🔎 Get items without email by page of ${pageSize} (options ${chalk.bold(
		'page'
	)} & ${chalk.bold('skip')})`, () => {
		return new Promise((resolve, reject) => {
			const loop = () => {
				expect(page).to.be.below(
					Math.ceil(data.length / pageSize) + 1,
					'Oops, we looped over data length'
				);
				return Promise.props({
					page: adapter.findMany(
						tableName,
						{
							email: undefined,
						},
					...
					.then(queryRes => {
						expect(queryRes.page).to.be.deep.eql(
							queryRes.skip,
							'"page" & "skip" should return same data'
						);
						...
						expect(entities)
							...
							.that.have.lengthOf.below(
								pageSize + 1,
								`Sets should be at most ${pageSize} items length`
							);
	...
	it('🔎 Find all items', () => {
		return adapter
			...
			.then(items => {
				expect(items)
					.to.be.a.set.of.dataStoreEntity(adapter)
					.that.have.lengthOf(data.length + 1);
				allItems = items;
			});
	});
	it(`❌ Delete 2 pages (options ${chalk.bold('page')} & ${chalk.bold(
		'limit'
	)})`, () => {
		return adapter
			.deleteMany(
			...
			.then(items => {
				expect(items)
					.to.be.a.set.of.dataStoreEntity(adapter)
					.that.have.lengthOf(data.length + 1 - (20 + 5));
				const idxRemoved = l([])
					.concat(
