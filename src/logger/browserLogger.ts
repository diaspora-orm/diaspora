...
export class BrowserLogger extends Logger {
	...
	public constructor() {
		...
		this.silly = this.bindConsoleFct(
			ELoggingLevel.Silly,
			'info',
			'Diaspora: silly  '
		);
		this.verbose = this.bindConsoleFct(
			ELoggingLevel.Verbose,
			'info',
			'Diaspora: verbose'
		);
		this.debug = this.bindConsoleFct(
			ELoggingLevel.Debug,
			'info',
			'Diaspora: debug  '
		);
		this.log = this.bindConsoleFct(
			ELoggingLevel.Log,
			'log',
			'Diaspora: log    '
		);
		this.warn = this.bindConsoleFct(
			ELoggingLevel.Warn,
			'warn',
			'Diaspora: warn   '
		);
		this.error = this.bindConsoleFct(
			ELoggingLevel.Error,
			'error',
			'Diaspora: error  '
		);
	 ...
	 */
	private bindConsoleFct(
		minLevel: ELoggingLevel,
		fctName: string,
		...args: any[]
	) {
		return ( ...logArgs: any[] ) => {
